...
def create_url_combination(filter_field):
    ...
    url_list = [
        f"https://ws-public.interpol.int/notices/v1/yellow?&{filter_field}=^{combination}.*&resultPerPage=160&page=1"
        for combination in combinations
    ]

    url_list.extend(
        [
            f"https://ws-public.interpol.int/notices/v1/yellow?&{filter_field}=^[a-z][^a-z].*&resultPerPage=160&page=1",
            f"https://ws-public.interpol.int/notices/v1/yellow?&{filter_field}=^[^a-z].*&resultPerPage=160&page=1",
            f"https://ws-public.interpol.int/notices/v1/yellow?&{filter_field}=^[a-z]{{0,1}}$.*&resultPerPage=160&page=1",
        ]
    )
    return url_list
...
def get_current_criminal_data(data_lst):
    # Получаем данные по конкретному пропавшему
    ...
    for i in range(total_wanted):
        ...
        if not skip_reading_profiles:
            # Делаем запрос к профилю пропавшего для получения дополнительной информации
            extra = requests.get(url=link, headers=HEADERS, proxies=PROXIES).text
            extra = json.loads(extra)
            place = extra.get("place")
            distinguishing_marks = extra.get("distinguishing_marks")
            date_of_event = extra.get("date_of_event")

        if data_lst.get("query").get("name"):
            # Если фильтруем по фамилии, то добавляем в список данные по каждому пропавшему
...
def write_files():
    ...
    with open("yellow_notices_urls.json", "w") as file:
        json.dump(result_url_dict, file, indent=4, ensure_ascii=False)

    with open("yellow_notices_{current_time}.json", "w") as file:
        json.dump(missing_data_list, file, indent=4, ensure_ascii=False)
